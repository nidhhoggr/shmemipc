...
import (
	"context"
	"encoding/binary"
	"github.com/joe-at-startupmedia/shmemipc/protos"
...
func (smp *ShmProvider) Receive(ctx context.Context,
	...
	for !smp.closed {
		...
		request := &protos.ShmemRequestMessage{}
		...
		response := &protos.ShmemResponseMessage{Data: responseData, Status: int32(status), StatusMessage: statusMessage}
...
func (smp *ShmProvider) Send(ctx context.Context, data []byte,
	...
	smp.buffer.ResizeNoShrink(0)
	request := protos.ShmemRequestMessage{Data: data, Metadata: requestMetadata}
	...
	var response *protos.ShmemResponseMessage
	for {
		smp.waitforevent(smp.rdevent)

		// Read the response
		response = &protos.ShmemResponseMessage{}
